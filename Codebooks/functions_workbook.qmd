...
Functions have a similar structure to conditionals and loops. You initiate with the command *function*, specify 'arugments' that modify the function output in parentheses, and contain the body of code within curly braces. You should name a function, as functions are objects in R. You also need to return an object at the end of R code.

Function are great for a number of reasons. They:
...
Let's covert our max code from last time into a function. Call the function **my_max**.

```{r}

  data <- c(10,40,3,12,18)
  cmax <- -Inf 
...
-   Functions run locally. That means that variables defined in functions are only available while the funciton is running. This is a good thing. Imagine if your object kept getting overwritten because you used a function that had an object with the same name as yours.

-   For example, the variables data (automatically assigned to the input) and cmax only exist while the mymax() function is running.

-   These values are assigned from scratch each time the function is called.
...
Just make sure you're designing functions to fit the objects you want to work with. These functions will not work on data frames as they currently are. R reads arguments by position, so if you don't address each argument by name, you better know the order the arguments are in.
...
-   Conditionals can help us with this. We can make sure code only runs if our arguments are appropriately used (e.g., people enter a vector for vmax() and not something else).
...
1.) Build a function similiar to vmax for minimums now. Give it at least two arguments (the second is up to you, it can be n or something else you think of).
